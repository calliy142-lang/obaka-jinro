const API = {
    ...
    async sendAction(roomCode, playerId, action) {
        ...
        if (!res.ok) {
            const data = await res.json().catch(() => ({}));
            throw new Error(data.detail || "アクションの送信に失敗しました");
        }
    ...
    async sendVote(roomCode, playerId, targetId) {
        ...
        if (!res.ok) {
            const data = await res.json().catch(() => ({}));
            throw new Error(data.detail || "投票の送信に失敗しました");
        }
    ...
    async updateSettings(roomCode, settings) {
        const res = await fetch(`/api/room/${roomCode}/settings`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (!res.ok) {
            const data = await res.json().catch(() => ({}));
            throw new Error(data.detail || "設定の更新に失敗しました");
        }

        return await res.json();
    },

    async rematchRoom(roomCode, hostPlayerId) {
        ...
        if (!res.ok) {
            const data = await res.json().catch(() => ({}));
            throw new Error(data.detail || "再戦の開始に失敗しました");
        }
    ...
    async startGame(roomCode, hostPlayerId) {
        ...
        if (!res.ok) {
            const data = await res.json().catch(() => ({}));
            throw new Error(data.detail || "ゲームの開始に失敗しました");
        }
