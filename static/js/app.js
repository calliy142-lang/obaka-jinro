...
function buildRoleActionUI(info) {
    ...
    if (role === "ボマー") {
        ...
        const bombMode = document.createElement("select");
        bombMode.id = "bombModeSelect";
        bombMode.style.padding = "6px";

        const plant = document.createElement("option");
        plant.value = "plant";
        plant.innerText = "爆弾を仕掛ける";

        const detonate = document.createElement("option");
    ...
    if (role === "シーフ") {
        actionHelp.innerText = "選択したプレイヤーを殺害し、その役職を盗みます。";
    }

    if (role === "ねずみ") {
        actionHelp.innerText = "1回だけ使用できます。調査結果は次の昼に全員へ公表されます。";
    }

    if (role === "挑発者") {
        actionHelp.innerText = "対象の次の昼の票数を+2します。残り2回まで使用できます。";
    }

    if (role === "ドクター") {
        actionHelp.innerText = "対象が夜に死亡した場合、蘇生できます。同じ対象を2夜連続では選べません。";
    }

    if (role === "ポリス") {
        actionHelp.innerText = "対象の夜能力を封じます。同じ対象を2夜連続では選べません。";
    }

    if (role === "トラッパー") {
        actionHelp.innerText = "対象の家に罠を仕掛け、そこを訪れた人のうち1人をランダムに封じます。";
    }

    if (role === "ルックアウト") {
        actionHelp.innerText = "対象の家を訪れたプレイヤーを確認します。";
    }

    if (role === "インベスティゲーター") {
        actionHelp.innerText = "対象の役職候補を2つに絞り込みます。";
    }

    if (role === "トラッカー") {
        actionHelp.innerText = "対象が夜に訪れた家を確認します。";
    }

    if (role === "ゴースト") {
        actionHelp.innerText = "対象の家にろうそくを置き、投票で追放された場合は次の夜に復讐します。";
    }

    if (role === "バカ") {
        actionHelp.innerText = "あなたには別のイノセント役職に見えていますが、実際には能力を持ちません。";
    }


    if (role === "ブレイマー") {
        actionHelp.innerText = "対象の死亡・追放時の役職表示をインポスターに見せます。残り2回まで使用できます。";
    }

    if (role === "クリーナー") {
        actionHelp.innerText = "対象が死亡・追放された際、その役職を不明にします。";
    }

    if (role === "シリアルキラー") {
        actionHelp.innerText = "対象を殺害します。ポリスやトラッパーでは止まりません。";
    }

    if (role === "サバイバー") {
        actionHelp.innerText = "夜の行動はありません。殺害されても最大3回まで復活します。";
    }
}

async function handleActionSubmit() {
    const targetId = document.getElementById("targetSelect").value;
    const displayedRole = document.getElementById("roleName").dataset.role || "";
...
function renderPrivateReports(info) {
    const box = document.getElementById("privateReportBox");
    if (!box) return;
...
function renderVoteArea(info) {
    ...
    if (info.phase !== "DAY" || !info.alive || info.action_submitted) {
...
async function handleVoteSubmit() {
    const targetId = document.getElementById("voteSelect").value;
...
function renderResultActions(info) {
    const panel = document.getElementById("resultActions");
    const winnerText = document.getElementById("resultWinnerText");
    const rematchButton = document.getElementById("rematchButton");
    const waitingText = document.getElementById("rematchWaitingText");
    ...
    if (winner === "innocent") winner = "イノセント";
    if (winner === "imposter") winner = "インポスター";
    if (winner === "neutral") winner = "ニュートラル";
    if (winner === "survivor") winner = "サバイバー";
    if (winner === "draw") winner = "引き分け";

    if (winnerText) {
        winnerText.innerText = `勝利: ${winner}`;
    }

    // ホストだけ再戦ボタンを押せる。
    if (rematchButton) {
        rematchButton.style.display = info.is_host ? "block" : "none";
    }

    if (waitingText) {
        waitingText.style.display = info.is_host ? "none" : "block";
    }
}
...
async function updateGameState() {
    ...
    try {
        ...
        roleName.innerText = `あなたの役職: ${info.displayed_role}`;
        roleName.dataset.role = info.displayed_role;

        document.getElementById("statusText").innerText =
            info.alive ? "状態: 生存" : "状態: 死亡";
